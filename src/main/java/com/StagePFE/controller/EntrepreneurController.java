...
public class EntrepreneurController {
	...
	public RedirectView inscrireEntrepreneur(Model model, @ModelAttribute("entrepreneur") Entrepreneur e,
			@RequestParam("profile_photo") MultipartFile photo, @RequestParam(name = "mdp") int mdp,
			@RequestParam(name = "mdpConfirmation") int mdpConfirmation, HttpServletRequest request,
...
//		e.setPhoto(fileDownloadUri);
		...
		user.setPassword(bcp.encode(Integer.toString(mdp)));
		user.setActive(true);
	...
	public RedirectView modifierProfileEntrepreneur(Model model, @ModelAttribute("entrepreneur") Entrepreneur e,
			@RequestParam("profile_photo") MultipartFile file, @RequestParam(name = "nouveaumdp") String nouveaumdp)
			throws IOException {
		...
		Entrepreneur entrepreneur = entrepreneurRepository.findByEmail(httpServletRequest.getRemoteUser()).get(0);
		entrepreneur.setAdresse(e.getAdresse());
		entrepreneur.setDescription(e.getDescription());
		entrepreneur.setEmail(e.getEmail());
		entrepreneur.setNom(e.getNom());
		entrepreneur.setNomEntreprise(e.getNomEntreprise());
		entrepreneur.setPhoneNmbr(e.getPhoneNmbr());
		entrepreneur.setPrenom(e.getPrenom());
		entrepreneur.setPhoto(e.getPhoto());
	...
	public RedirectView photoEntrepreneur(Model model, @RequestParam("profile_photo") MultipartFile photo)
			throws IOException {
		User user = userRepository.findByUsername(httpServletRequest.getRemoteUser());
		if (user == null)
			return new RedirectView("/login");
		...
		if (photo.getSize() == 0) {
			...
			entrepreneur
					.setPhotodata(Files.readAllBytes(Paths.get("src/main/resources/static/imgs/entrepreneur2.jpg")));
		} else {
	...
	public RedirectView publierAnnonce(Model model, @RequestParam(name = "page", defaultValue = "0") int page,
		...
		Annonce annonce = new Annonce(null, entrepreneur, null, null, a.getTitre(), a.getDescription(), new Date(),
				null, true, a.getLieu());
		if (a.getLieu() == "") {
